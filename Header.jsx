...
//Search
 // Gọi API backend khi nhập từ khóa
  useEffect(() => {
  ...
  // Chọn gợi ý → chuyển trang
  const handleSelect = (item) => {
    setQuery(item);
    setShowDropdown(false);
    navigate(`/search/allresults?q=${encodeURIComponent(item)}`);
  };
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      if (activeIndex >= 0 && suggestions[activeIndex]) {
        ...
        navigate(`/search/allresults?q=${encodeURIComponent(query)}`);
      }
    }
  };
  ...
  return (
    <div className="header flex">
        ...
        <ul className="suggest-box">
          {suggestions.map((item, index) => (
            <li
              key={index}
              className={
                "suggest-item " + (index === activeIndex ? "active" : "")
              }
      ...
      {showDropdown && suggestions.length === 0 && (
        <div className="suggest-empty">No results found</div>
      )}
    </div>
