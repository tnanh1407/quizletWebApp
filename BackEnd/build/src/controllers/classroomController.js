...
const addFlashcards = async (req, res, next) => {
  try {
    ...
    if (!classroom) return res.status(StatusCodes.NOT_FOUND).json({
...
const removeFlashcard = async (req, res, next) => {
  try {
    ...
    const classroom = await classroomService.removeFlashcard(classroomId, flashcardId);
    if (!classroom) return res.status(StatusCodes.NOT_FOUND).json({
      message: "Folder not found or flashcard not in classroom"
    });
    res.status(StatusCodes.OK).json(classroom);
  } catch (err) {
    next(err);
...
const removeFolder = async (req, res, next) => {
  try {
    ...
    const classroom = await classroomService.removeFlashcard(classroomId, folderId);
    if (!classroom) return res.status(StatusCodes.NOT_FOUND).json({
      message: "Folder not found or flashcard not in classroom"
    });
    res.status(StatusCodes.OK).json(classroom);
...
const removeMember = async (req, res, next) => {
  try {
    ...
    if (!classroom) {
      return res.status(StatusCodes.NOT_FOUND).json({
        message: "classroom not found or member not exists"
      });
    }
    res.status(StatusCodes.OK).json(classroom);
  } catch (err) {
...
const handleJoinRequest = async (req, res, next) => {
  try {
    ...
    const {
      user_id,
      action
    } = req.body; // action = "accept" | "reject"

    const classroom = await classroomService.handleJoinRequest(id, user_id, action);
    res.status(StatusCodes.OK).json(classroom);
  } catch (error) {
    next(error);
  }
...
const cancelJoinRequest = async (req, res, next) => {
  try {
    ...
    const updated = await db.collection("classrooms").findOne({
      _id: new ObjectId(id)
    });
