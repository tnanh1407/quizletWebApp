...
const CLASSROOM_COLLECTION_SCHEMA = Joi.object({
  ...
  pending_users: Joi.array().items(Joi.object({
    user_id: Joi.string().required(),
    username: Joi.string().required(),
    avatar: Joi.string().allow("")
  })),
...
const validateClassroom = data => CLASSROOM_COLLECTION_SCHEMA.validateAsync(data, {
  abortEarly: false
});
const getAll = async () => {
  try {
...
const getById = async id => {
  const db = GET_DB();
  return await db.collection(CLASSROOM_COLLECTION_NAME).findOne({
    _id: new ObjectId(id)
  });
};
const createNew = async data => {
  const autoData = {
    ...
    flashcard_count: Array.isArray(data.flashcards) ? data.flashcards.length : 0,
  ...
  const db = GET_DB();
  const result = await db.collection(CLASSROOM_COLLECTION_NAME).insertOne(validData);
...
const deleteById = async id => {
  const db = GET_DB();
  const result = await db.collection(CLASSROOM_COLLECTION_NAME).updateOne({
    _id: new ObjectId(id)
  }, {
    $set: {
...
const addFlashcards = async (classroomId, flashcardIds) => {
  ...
  const updatedFlashcards = [...new Set([...(classroom.flashcards || []), ...flashcardIds])];
  await db.collection(CLASSROOM_COLLECTION_NAME).updateOne({
...
const addFolders = async (classroomId, folderIds) => {
  ...
  const classroom = await getById(classroomId);
  if (!classroom) return null;
  const updatedFolders = [...new Set([...(classroom.folders || []), ...folderIds])];
  await db.collection(CLASSROOM_COLLECTION_NAME).updateOne({
    _id: new ObjectId(classroomId)
  }, {
    $set: {
...
const addMember = async (classroomId, member) => {
  ...
  const exists = (classroom.members || []).some(m => m.user_id === member.user_id);
...
const addMemberByEmail = async (classroomId, email) => {
  ...
  const classroom = await db.collection(CLASSROOM_COLLECTION_NAME).findOne({
    _id: new ObjectId(classroomId)
  });
  if (!classroom) throw new Error("Classroom not found");

  //  Kiểm tra nếu user đã là thành viên
  const alreadyMember = classroom.members.some(m => m.user_id === user._id.toString());
...
const requestJoin = async (classroomId, user) => {
  ...
  const isPending = (classroom.pending_users || []).some(p => p.user_id === user.user_id);
...
const handleJoinRequest = async (classroomId, userId, action) => {
  ...
  const classroom = await getById(classroomId);
  if (!classroom) throw new Error("Classroom not found");
  const pendingUser = (classroom.pending_users || []).find(u => u.user_id === userId);
