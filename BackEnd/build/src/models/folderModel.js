...
const validateFolder = data => {
  return FOLDER_COLLECTION_SCHEMA.validateAsync(data, {
    abortEarly: false
  });
};
const getAll = async () => {
...
const getById = async id => {
  const db = GET_DB();
  return await db.collection(FOLDER_COLLECTION_NAME).findOne({
    _id: new ObjectId(id)
  });
};
const createNew = async (data, user) => {
  ...
  const db = GET_DB();
  const result = await db.collection(FOLDER_COLLECTION_NAME).insertOne(validData);
  return await db.collection(FOLDER_COLLECTION_NAME).findOne({
...
const deleteById = async id => {
  try {
    const db = GET_DB();
    const result = await db.collection(FOLDER_COLLECTION_NAME).updateOne({
      _id: new ObjectId(id)
    }, {
      $set: {
        delete_folder: true
...
const addFlashcards = async (folderId, flashcardIds) => {
  ...
  const updatedFlashcards = [...new Set([...(folder.flashcards || []), ...flashcardIds])];
  await db.collection(FOLDER_COLLECTION_NAME).updateOne({
