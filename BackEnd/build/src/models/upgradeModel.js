...
const UPGRADE_COLLECTION_NAME = "upgrade";

// Schema: chỉ bắt buộc title + content
const UPGRADE_COLLECTION_SCHEMA = Joi.object({
...
const validateBeforeCreate = data => {
  return UPGRADE_COLLECTION_SCHEMA.validateAsync(data, {
    abortEarly: false
  });
};
...
const getAll = async () => {
  try {
    const db = GET_DB();
    return await db.collection(UPGRADE_COLLECTION_NAME).find({}).toArray();
  } catch (error) {
...
const createNew = async (data, user) => {
  try {
    ...
    const db = GET_DB();
    const result = await db.collection(UPGRADE_COLLECTION_NAME).insertOne(validData);

    // Lấy flashcard vừa tạo
    const newFlashCard = await db.collection(UPGRADE_COLLECTION_NAME).findOne({
...
const getById = async id => {
  try {
    const db = GET_DB();
    return await db.collection(UPGRADE_COLLECTION_NAME).findOne({
      _id: new ObjectId(id)
    });
  } catch (error) {
    throw new Error(error);
...
const updateById = async (id, data) => {
  try {
    const db = GET_DB();
    const existing = await db.collection(UPGRADE_COLLECTION_NAME).findOne({
      _id: new ObjectId(id)
    });
    if (!existing) {
    ...
    await db.collection(UPGRADE_COLLECTION_NAME).updateOne({
      _id: new ObjectId(id)
    }, updateOps);
    return await db.collection(UPGRADE_COLLECTION_NAME).findOne({
      _id: new ObjectId(id)
...
const deleteById = async id => {
  try {
    const db = GET_DB();
    const result = await db.collection(UPGRADE_COLLECTION_NAME).updateOne({
      _id: new ObjectId(id)
    }, {
      $set: {
...
export const upgradeModel = {
  UPGRADE_COLLECTION_NAME,
  UPGRADE_COLLECTION_SCHEMA,
  getAll,
  createNew,
  getById,
  updateById,
