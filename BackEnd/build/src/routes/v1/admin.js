...
router.get("/overview", async (req, res) => {
  try {
    ...
    // Lấy tổng số
    const [usersCount, flashcardsCount, classroomsCount] = await Promise.all([db.collection("users").countDocuments(), db.collection("flashcards").countDocuments(), db.collection("classrooms").countDocuments()]);
    ...
    const activities = [...flashcards.map(f => ({
      type: "Tạo bộ thẻ",
      name: f.title,
    ...
    }))].sort((a, b) => new Date(b.createAt) - new Date(a.createAt)).slice(0, 10);
    res.json({
      totals: {
        users: usersCount,
